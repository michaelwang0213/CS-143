...
state_data = pd.read_csv("count_states.csv")
state_data = state_data[['flair', 'pos%', 'neg%']]
...
diff_data = dict(zip(state_data.state, state_data.Negative))
...
diff_colors = {}
statenames = []
pos_cmap = plt.cm.Blues # use 'hot' colormap
neg_cmap = plt.cm.Reds # use 'hot' colormap
diff_cmap = plt.cm.Purples # use 'hot' colormap

pmin = 0.3; pmax = 0.7 # set range.
nmin = 0.7; nmax = 1
dmin = 0; dmax = 0.7
for shapedict in m.states_info:
    ...
    if statename not in ['District of Columbia', 'Puerto Rico']:
        ...
        diff = neg - pos
        pos_colors[statename] = pos_cmap(1. - np.sqrt(( pos - pmin )/( pmax - pmin)))[:3]
        neg_colors[statename] = neg_cmap(1. - np.sqrt(( neg - nmin )/( nmax - nmin)))[:3]
        diff_colors[statename] = diff_cmap(1. - np.sqrt(( diff - dmin )/( dmax - dmin)))[:3]
...
plt.savefig("mydifferencemap.png")
...
story = pd.read_csv("count_sscore.csv")
story= story[['sscore', 'pos%', 'neg%']]
...
story = pd.read_csv("count_cscore.csv")
story= story[['cscore', 'pos%', 'neg%']]
